    ...
    with get_db() as conn:
        ...
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXIST jobs_attempts (
                ...
                status TEXT NOT NULL CHECK(status IN ('IN_PROGRESS', 'SUCCESS', 'APP_ERROR', 'FENCED_OUT', 'STALE_TIME')),
...
def insert_job(job_id: str, type: str, payload: str, max_attempts: int = 3) -> None:
    with get_db() as conn:
        conn.execute("BEGIN IMMEDIATE")
        conn.execute(
            "INSERT INTO jobs (id, type, payload, max_attempts) VALUES (?, ?, ?, ?)",
            (job_id, type, payload, max_attempts),
        )
        conn.execute("COMMIT")
...
def claim_next_pending_job(worker_id: str):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            """
            INSERT INTO job_attempts (id, job_id, worker_id, fence_token, attempt_number, started_at, status) 
